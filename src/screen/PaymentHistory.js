...
const PaymentHistory = ({navigation}) => {
  ...
  const fetchWalletHistory = async () => {
    try {
      ...
      const response = await axios.post(
        'https://righten.in/api/users/payment/history',
        postData,
        {
          headers: {
  ...
  const renderItem = ({ item }) => (
    <View style={[styles.card,item.mode === '1' && { borderLeftColor: '#009743'}]}>
      <View style={styles.row}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.flexRow}>
            <Text style={styles.serviceName}>UTR No</Text>
            <Text style={styles.subService}>
                {item.utr && item.utr.length > 12
                ? item.utr.slice(-12)
                : item.utr}
            </Text>
          ...
          <View style={styles.flexRow}>
            <Text style={styles.serviceName}>Mode</Text>
            <Text style={[
              styles.amount,
              item.mode === '1' ? styles.web : styles.app
            ]}>
              {item.mode === '1' ? 'Web' : 'APP'}
            </Text>
          </View>
          <View style={styles.flexRow}>
            <Text style={styles.serviceName}>{item.txn_id}</Text>
